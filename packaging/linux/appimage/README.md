...
`fulvid.desktop` here uses `Exec=fulvid %F`. Icon: `assets/fulvid.png`. `%F` is an unquoted field code so a file manager can pass one or more local paths. Those paths still stop at the Electrobun launcher until it forwards them; see [EXTERNAL-OPEN.md](../../../docs/EXTERNAL-OPEN.md).
