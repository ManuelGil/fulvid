...
**Session change markers:** while a document is open, the gutter can show lines that differ from that tab's in-memory session baseline (opened/saved/initial Untitled content). Modified and added use restrained amber and green theme tokens. Session Changes are an ephemeral presentation of baseline vs current content; Settings -> Editor display -> Show session changes (default on) shows or hides that presentation together - markers and the temporary hunk-scoped Before -> Now comparison opened from a marker. Disabling presentation does not stop baseline tracking used by the document lifecycle. The preview is derived from baseline vs current text, never owns the document, and is not persisted. Save clears markers by resetting the baseline and dismisses any open preview. Markers are not Git, not persistent history, and are not stored in IndexedDB draft recovery. The live Monaco model remains authoritative.
...
Fulvid does not run MDX or document HTML. It does not score notes, infer links, or use embeddings. There is no Command Palette. Quick Open (`Ctrl/Cmd+P`) opens a document by title, filename, or relative path in the open Folder. Monaco is the only editor. Preview and Export HTML share one inert renderer. Fulvid is not a plugin marketplace. **Extensions** are local packs under `userData/extensions` (Extension API v1) that compose through explicit capabilities into existing owners (`extension -> capability -> owner -> state`). Lua is the supported extension runtime (wasmoon / Wasm), not a second product: packs load `entry.lua` under host-enforced budgets. Packs may declare least-privilege Lua surfaces (`editor`, `document`, `decorations`) with snapshot -> reject-stale apply through existing owners. There is no filesystem capability for extensions, no declarative host-action path, and no generic `host.call`. Capability isolation is not an OS sandbox. Contract: [EXTENSIONS.md](./EXTENSIONS.md).
...
Line endings (LF or CRLF) belong to the document, not to the operating system. Opening a file keeps its endings. Untitled documents use Settings -> Files -> New document line endings (LF by default). The statusbar shows the active document's ending; click to switch. Save writes that ending. Changing it does not write until Save.

**Trim trailing whitespace** (Edit -> Trim trailing whitespace) removes spaces and tabs immediately before each line ending via Monaco edits (undoable). It does not change EOL, leading indentation, or Preview. Settings -> Files -> Trim trailing whitespace on Save is off by default; when on, the same edit runs immediately before a confirmed Save write, or after the Save As filename is confirmed (cancel does not trim). To *see* trailing whitespace while typing, use Settings -> Editor display -> Whitespace -> Always (Monaco `renderWhitespace`); Fulvid does not add a separate trailing-space highlighter.
