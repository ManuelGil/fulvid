...
Fulvid does not run MDX or document HTML. It does not score notes, infer links, or use embeddings. There is no Command Palette. Quick Open (`Ctrl/Cmd+P`) opens a document by title, filename, or relative path in the open Folder. Monaco is the only editor. Preview and Export HTML share one inert renderer. Fulvid is not a plugin marketplace. Local packs under `userData/extensions` compose through explicit capabilities into existing owners (`extension → capability → owner → state`). Declarative packs may declare host actions (`notify`, `createUntitledFromTemplate`). Packs that declare `lua` may load `entry.lua` in the Bun-host Lua/Wasm runtime (wasmoon 1.16.0, embedded PUC Lua 5.4.5) under host-enforced execution and memory budgets. Packs that also declare `editor` may use **EXPERIMENTAL** `editor.getSelection` / `editor.replaceSelection` (snapshot text → Lua → live Monaco apply); those APIs are not a stable public contract. There is no filesystem capability for extensions and no generic `host.call`. Capability isolation is not an OS sandbox; it is not a second application runtime. Extension Engine contract: [EXTENSIONS.md](./EXTENSIONS.md).
