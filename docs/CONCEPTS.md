...
Fulvid does not run MDX or document HTML. It does not score notes, infer links, or use embeddings. There is no Command Palette. Quick Open (`Ctrl/Cmd+P`) opens a document by title, filename, or relative path in the open Folder. Monaco is the only editor. Preview and Export HTML share one inert renderer. Fulvid is not a plugin marketplace. **Extensions** are local packs under `userData/extensions` (Extension API v1) that compose through explicit capabilities into existing owners (`extension -> capability -> owner -> state`). Declarative packs may declare host actions (`notify`, `createUntitledFromTemplate`). Lua is a supported extension runtime (wasmoon / Wasm), not a second product: packs may load `entry.lua` under host-enforced budgets. Packs that declare `editor` may use production `editor.getSelection` / `editor.replaceSelection` (text snapshot -> reject-stale Monaco apply). There is no filesystem capability for extensions and no generic `host.call`. Capability isolation is not an OS sandbox. Contract: [EXTENSIONS.md](./EXTENSIONS.md).
