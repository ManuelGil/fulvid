...
| **Annotation** | Temporary plain-text note on a tracked position in the open document's live Monaco model. Session-local and document-local; not source, not persistent, not searchable. Show/Hide is presentation only. Navigate menu + Quick Actions. See [ANNOTATIONS.md](./ANNOTATIONS.md) |
