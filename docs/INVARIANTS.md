...
| Extension UI boundary | Path: `extension → declared capability/command → existing owner → presentation`. An extension is never an owner and never a generic host bridge. Declarative discovery loads `userData/extensions` (`api: 0`); invalid packs fail in isolation and do not block valid neighbors. Host actions today: `notify`, `createUntitledFromTemplate`, plus host-only `lua` invoke. Lua packs may use `commands.register` / `ui.notify` under resource budgets (source size, command count, notify length, wall-clock execution, Wasm heap). Experimental `editor` APIs (`getSelection` / `replaceSelection`) transform the **primary selection** only through Monaco via snapshot/apply (snapshot = text; apply = live primary range on the active editor) — Lua is not the owner of live text, dirty, or undo. There is no full-buffer `editor.get`, no filesystem capability for Lua, no `host.call`, and no Monaco/host objects in the guest. Capability isolation ≠ OS sandbox. Icon identifiers are closed declarative names (`appIcons` / `CommandIcon`). Repo `extensions/` fixtures document the declarative boundary and are not the load path; experimental Lua fixtures under `tests/extensions/fixtures/` are disposable. Do not fix a UI bug by inventing a second owner of the same behavior |
