...
| Extension UI boundary | Path: `extension → declared capability/command → existing owner → presentation`. An extension is never an owner and never a generic host bridge. The Extension Engine orchestrates capabilities; it does not own filesystem, document, editor, window, search, graph, or renderer authority. Declarative + Lua surfaces, experimental `editor` snapshot/apply, budgets, absent-by-design list, contract tests, and accepted limitations (capability isolation ≠ OS sandbox; editor live-apply TOCTOU): [EXTENSIONS.md](./EXTENSIONS.md). Do not fix a UI bug by inventing a second owner of the same behavior |
