...
Packaging variants may change only `Exec`. Name, Comment, Icon, StartupWMClass, Categories, and MIME stay the same. MIME is `text/markdown;text/x-markdown;` only. Do not add `.mdx` as MIME.
...
The first public tag can be attached by hand from workflow artifacts or a local Linux set, using [releases/v0.1.0.md](./releases/v0.1.0.md) as the body. Do not flip the gate just to finish a documentation pass.
